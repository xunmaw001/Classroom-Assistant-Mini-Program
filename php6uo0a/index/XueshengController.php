...
class XueshengController extends CommonController {
    ...
    public function login(){
        ...
        $password = isset($_REQUEST['password'])?$_REQUEST['password']:"";
                        $sql = "select * from `xuesheng` where `xuehao` = '".$username."' and `mima` = '".$password."'";
    ...
    public function register(){
        ...
        $postData = json_decode($tmpData,true);
                        $colum = "xuehao";
        $trues = "select * from `xuesheng` where `xuehao` = '".$postData[$colum]."'";
        ...
        if($result->num_rows<1){
			...
			$key = implode(',',$keyArr);
			$v = implode(',',$valArr);
                        $sql = "INSERT INTO `xuesheng` (`id`,".$key.") VALUES (".time().",".$v.")";
                        $result = table_sql($sql);
    ...
    public function resetPass(){
        $username = input('post.username');
                        $counts = "select * from `xuesheng` where xuehao = '".$username."'";
        $cotte = table_sql($counts);
        if($cotte->num_rows<1){
            exit(json_encode(['code'=>500,'msg'=>"用户名错误。"]));
        }
        $sql = "update xuesheng set mima = '123456' where xuehao = '".$username."'";
                                                                                                                                                        		        $result = table_sql($sql);
    ...
    public function page(){
        ...
        if ($base['isAdmin']!=1){
            ...
            if (isset($_SESSION[$md5]) && in_array($colum, $columData)){
                if($base['tablename'] == '${column.authTable}'){
                    $where .= " and `xuehao` = '".$_SESSION[$md5]."'";
                }
            }
        }
                                                                                                                                                        		        
                                        $sql = "select * from `xuesheng` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $startCount = ($page-1)*$limt;
                    $lists = "select * from `xuesheng` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
                ...
                $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `xuesheng` (`id`,".$k.") VALUES (".time().",".$v.")";
                $result = table_sql($sql);
		        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function add(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                $sql = "INSERT INTO `xuesheng` (".$k.") VALUES (".$v.")";
        $result = table_sql($sql);
		        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE xuesheng SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function delete(){
        ...
        $sql = "delete from xuesheng WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                        ...
                        $sql = "select * from `xuesheng` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            while($row = $result->fetch_assoc()) {
    ...
    public function remind($columnName,$type){
        ...
        $sql = "select * from `xuesheng` where 1 ".$where;
        $result = table_sql($sql);
