<?php
session_start();
class JiaoxueneirongController extends CommonController {
    ...
    public function page(){
        ...
        $startCount = ($page-1)*$limt;
                    $lists = "select * from `jiaoxueneirong` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function lists(){
        ...
        if ($base['isAdmin']!=1){
            ...
            if (isset($base['success'][$colum]) && in_array($colum, $columData)){
                $where .= " and `jiaoshigonghao` = '".$base['success'][$colum]."'";
            }
        }
                                                                                        		                                $sql = "select * from `jiaoxueneirong`".$where;
        $count = table_sql($sql);
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
                ...
                $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `jiaoxueneirong` (".$k.") VALUES (".$v.")";
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE jiaoxueneirong SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function delete(){
        ...
        $sql = "delete from jiaoxueneirong WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                ...
                $this->andone('jiaoxueneirong',$id,$column='clicknum',$num=1);
                        $clicktime = "update jiaoxueneirong set clicktime = ".date('Y-m-d H:i:s',time())." where ".$where;
        table_sql($clicktime);
                $sql = "select * from `jiaoxueneirong` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function detail($id=false){
		...
		$limt = isset($_REQUEST['limit'])?$_REQUEST['limit']:"5";
				$sort = 'clicknum';
				$lists = "select * from `jiaoxueneirong` order by ".$sort." desc limit 0,".$limt;
		$result = table_sql($lists);
		$arrayData = array();
		if ($result->num_rows > 0) {
			while ($datas = $result->fetch_assoc()){
    ...
    public function remind($columnName,$type){
        ...
        $sql = "select * from `jiaoxueneirong` where 1 ".$where;
        $result = table_sql($sql);
