<?php
session_start();
class XueshengqiandaoController extends CommonController {
    ...
    public function page(){
            ...
            if (isset($_SESSION[$md5]) && in_array($colum, $columData)){
                if($base['tablename'] == 'jiaoshi'){
                    $where .= " and `jiaoshigonghao` = '".$_SESSION[$md5]."'";
                                        ...
                                        $sql = "select * from `xueshengqiandao` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $startCount = ($page-1)*$limt;
                    $lists = "select * from `xueshengqiandao` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function lists(){
        ...
        if ($base['isAdmin']!=1){
            ...
            if (isset($base['success'][$colum]) && in_array($colum, $columData)){
                $where .= " and `xuehao` = '".$base['success'][$colum]."'";
            }
        }
                                                                                                                $token = $this->token();
        $base = json_decode(base64_decode($token),true);
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
                ...
                $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `xueshengqiandao` (".$k.") VALUES (".$v.")";
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE xueshengqiandao SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function delete(){
        ...
        $sql = "delete from xueshengqiandao WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                        ...
                        $sql = "select * from `xueshengqiandao` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function remind($columnName,$type){
        ...
        $sql = "select * from `xueshengqiandao` where 1 ".$where;
        $result = table_sql($sql);
